...
namespace BlackMGMod.MemoryAccessing
{
    class MemoryAccesser
    {
        ...
        public void ReplaceCode(byte[] code, IntPtr codeAddr, int replacedCodeSize)
        {
            if (replacedCodeSize < code.Length) return;

            var newCode = new byte[replacedCodeSize];
            for(var i=0; i<replacedCodeSize; i++)
            {
                newCode[i] = NOP_INSTRUCTION;
            }
            Array.Copy(code, newCode, code.Length);
            CodeAlterationWriteBytes(codeAddr, newCode);
        }
        ...
        public IntPtr InsertCode(byte[] codeToInsert, IntPtr insertAddress)
        {
            ...
            Array.Copy(jumpToNewCode, replacementCode, jumpToNewCode.Length);//TODO use replace code function
